...
export default function ProductCard({ product }: { product: Product }) {
  ...
  return (
    <div className="group relative holographic-border rounded-card overflow-hidden bg-pharma-850/80 backdrop-blur-sm hover:bg-pharma-800 transition-all duration-300">
      {/* Top accent bar */}
      <div className="h-1 bg-brand-elysion" />

      <div className="p-6">
        {/* Brand + Badge */}
        <div className="flex items-start justify-between mb-3">
          <span className="text-xs font-bold tracking-widest uppercase text-brand-elysion">
            ELYSION
          </span>
      ...
      {/* Hover glow */}
      <div className="absolute inset-0 rounded-card opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none" style={{ boxShadow: 'inset 0 0 40px rgba(149,117,205,0.08)' }} />
    </div>
  )
}
